...
// Done is a sentinel value used to stop iteration in Decoder.DecodeMap and Decoder.DecodeSlice.
var Done = errors.New("done")

// DecodeMap decodes the value pointed by the decoder as a map.
//
// If the callback returns the special sentinel Done, the iteration stops
// immediately, the remaining elements are skipped and DecodeMap returns nil.
// If any other error is returned, the iteration stops immediately and DecodeMap
// returns that error.
//
// Returns an error if the database is malformed or if the pointed value is not a map.
func (d *Decoder) DecodeMap(cb func(key string, value *Decoder) error) error {
	...
	for i := uint(0); i < size; i++ {
		...
		if cbErr != nil {
			if cbErr == Done {
				// Skip the unvisited elements and shadow the error:
...
//
// If the callback returns the special sentinel Done, the iteration stops
// immediately, the remaining elements are skipped and DecodeSlice returns nil.
// If any other error is returned, the iteration stops immediately and DecodeSlice
// returns that error.
//
// Returns an error if the database is malformed or if the pointed value is not a slice.
func (d *Decoder) DecodeSlice(cb func(value *Decoder) error) error {
